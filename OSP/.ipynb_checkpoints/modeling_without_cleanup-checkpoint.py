...
class rnn_no_cleanup(Layer):
    ...
    def call(self, inputs):
        ...
        for t in range(1, self.cfg.n_timesteps + 1):
            ...
            p = self.cfg.tau * (hp + pp + self.bias_p) + (1 -
                                                          self.cfg.tau) * mem_p
...
class rnn_no_cleanup_trim(Layer):
    # Trimmed all connection to P, except S
    # I.e. It takes pure semantic input only
    def __init__(self, cfg, **kwargs):
        super(rnn_no_cleanup_trim, self).__init__(**kwargs)
    ...
    def call(self, inputs):
        o_input, s_input = tf.split(inputs,
                                    [self.cfg.o_input_dim, self.cfg.pho_units],
                                    2)
        self.input_p_list = []
        self.act_p_list = []
        ...
        for t in range(1, self.cfg.n_timesteps + 1):
            ...
            p = s_input[:, t - 1, :]  # p-only get semantic input
