...
class MyModel(tf.keras.Model):
    ...
    def task_triangle(self, inputs, training=None):
        ...
        for t in range(self.n_timesteps):
            ...
            # Update activations
            [self._update_activations(t + 1, x, clamp_trash=True) for x in self.ACTIVATION_ARRAY_NAMES]
    ...
    def _init_a_tensor_array(self, name: str, shape: tuple):
        setattr(
    ...
    def _init_tensor_arrays(self):
        ...
        # Set inputs to 0
        [
            self._init_a_tensor_array(x, shape=self.shapes[x])
    ...
    def _update_activations(self, timetick: int, act: str, clamp_trash: bool=False):
        """Update the activation when it is at informative ticks (from O)"""
        sum_input = getattr(self, f"input_{act}").read(timetick)
        
        if clamp_trash and timetick in self.TRASH_TICKS[act]:
            activation = tf.zeros_like(sum_input)
        else:
