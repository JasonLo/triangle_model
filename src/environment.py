...
class Stage:
    ...
    def __init__(self, name: str, tasks: List[Task], stage_sample: int, task_probability_start: List[float], task_probability_end: List[float] = None):
        self.name = name
        self.tasks = tasks
        self.stage_sample = stage_sample
        self.task_probability_start = task_probability_start
        self.task_probability_end = task_probability_start if task_probability_end is None else task_probability_end
...
class Experience:
    ...
    def from_config(cls, config: EnvironmentConfig):
        ...
        stages = [Stage("one", tasks, config.total_sample, config.tasks_ps)] # Stationary 
    ...
    def plot_corpus(self, scale_x=1000):
        ...
        for i, stage in enumerate(self.stages):
            ...
            for j, task in enumerate(stage.tasks):
                ...
                ax.set_title(f'Stage {i+1}: Corpus opening progression (%) in {stage.name}')
